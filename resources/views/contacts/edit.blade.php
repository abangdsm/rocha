                    ...
                    <form method="POST" action="{{ route('contacts.update', $contact) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <textarea name="address" rows="2" class="w-full border border-gray-300 rounded-lg p-2">{{ old('address', $contact->address) }}</textarea>
                            </div>

                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                                <textarea name="notes" rows="3" class="w-full border border-gray-300 rounded-lg p-2">{{ old('notes', $contact->notes) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="groups[]" multiple class="w-full border border-gray-300 rounded-lg p-2">
                                    @foreach($groups as $group)
                                        <option value="{{ $group->id }}" {{ $contact->groups->contains($group->id) ? 'selected' : '' }}>
                                            {{ $group->name }}
                                        </option>
                                    @endforeach
                                </select>
